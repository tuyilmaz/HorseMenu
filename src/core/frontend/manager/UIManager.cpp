...
namespace YimMenu
{
	void UIManager::AddSubmenuImpl(std::shared_ptr<Submenu>&& submenu)
	{
	...
	void UIManager::DrawImpl()
	{
		...
		for (size_t i = 0; i < m_Submenus.size(); ++i)
		{
			...
			ImU32 activeIconColor = IM_COL32(204, 0, 1, 255);
			ImU32 hoveredIconColor = IM_COL32(107, 5, 0, 255);
			...
			ImU32 activeTextColor = IM_COL32(204, 0, 1, 255);
			ImU32 hoveredTextColor = IM_COL32(107, 5, 0, 255);
	...
	std::shared_ptr<Submenu> UIManager::FindSubmenuByNameImpl(const std::string& name)
	{
		std::lock_guard lock(m_Mutex);

		for (auto& submenu : m_Submenus)
		{
			if (submenu->m_Name == name)
				return submenu;
		}
